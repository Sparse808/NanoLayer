...
func main() {
	...
	rect := NewDraggableRect(50, 50, *gridBackground)

	noLayout := container.NewWithoutLayout(gridBackground, rect)

	title := widget.NewLabel("Tools")
	but1 := widget.NewButton("box", func() {
		fmt.Println("pressed")

		noLayout.Objects = append(noLayout.Objects, NewDraggableRect(70, 70, *gridBackground))
	})

	toolsSection := container.NewVBox(title, but1)

	myLayout := container.NewHBox(layout.NewSpacer(), noLayout, layout.NewSpacer(), toolsSection)
...
func NewDraggableRect(x, y float32, grounds canvas.Rectangle) *DraggableRect {
	dr := &DraggableRect{
